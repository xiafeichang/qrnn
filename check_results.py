...
def draw_plot(x_vars, x_title, x_var_name, target, transformer_file):
...
#        var_data_mean[i] = np.mean(inverse_transform((df_data.query(query_str_pT))[target], transformer_file, target))
    ...
    plt.legend()
    fig.savefig('plots/check_correction/{}_{}_{}.png'.format(EBEE, target, x_var_name))
    plt.close(fig)
...
draw_plot(pTs, '$p_T$', 'probePt', target, transformer_file)
draw_plot(etas, '$\eta$', 'probeScEta', target, transformer_file)
draw_plot(rhos, '$\\rho$', 'rho', target, transformer_file)
draw_plot(phis, '$\phi$', 'probePhi', target, transformer_file)
