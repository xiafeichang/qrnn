...
def trainClfp0t(df, features, target, clf_name, val_split=0.1, n_estimators=300, learning_rate=0.05, maxDepth=10, subsample=0.5, gamma=0, tree_method='auto', **kwargs):
    ...
    sample_weight_train = df_train.loc[:,'ml_weight']

    X_val = df_val.loc[:,features]
    Y_val = df_val.loc[:,'p0t_{}'.format(target)]
    sample_weight_val = df_val.loc[:,'ml_weight']

    clf = xgb.XGBClassifier(n_estimators=n_estimators,learning_rate=learning_rate,maxDepth=maxDepth,subsample=subsample,gamma=gamma,tree_method=tree_method)
    clf.fit(
        X_train, Y_train, 
...
def trainClf3Cat(df, features, target, clf_name, val_split=0.1, n_estimators=500, learning_rate=0.05, maxDepth=10, gamma=0, tree_method='auto', **kwargs):
    ...
    clf = xgb.XGBClassifier(n_estimators=n_estimators,learning_rate=learning_rate,maxDepth=maxDepth,gamma=gamma,tree_method=tree_method)
    clf.fit(
        X_train, Y_train, 
        sample_weight=sample_weight_train,
        eval_set = [(X_train,Y_train), (X_val, Y_val)],
