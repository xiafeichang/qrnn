...
#        features = kinrho + variables[:variables.index(target)] 
        ...
        train_end = time.time()
        print('time spent in training {}: {} s'.format(target,train_end-train_start))

        # correct
        models_mc = 'combined_models/{}_{}'.format('mc', target)
        models_d = 'combined_models/{}_{}'.format('data', target)
        df_train['{}_corr'.format(target_raw)] = applyCorrection(models_mc, models_d, X, Y, diz=False)
        correct_end = time.time()
        print('time spent in correcting {}: {} s'.format(target,correct_end-train_end))

        
        #test
        X_test_raw = df_test_raw.loc[:,features]
        Y_test_raw = np.array(df_test_raw.loc[:,target])
        df_test_raw['{}_corr'.format(target_raw)] = applyCorrection(models_mc, models_d, X_test_raw, Y_test_raw, diz=False)
...
#            fig.savefig('plots/combined_{}_{}_{}.pdf'.format(data_key, target, str(i)))
    ...
    run_end = time.time()
    print('time spent in total: {} s'.format(run_end-run_start))
