...
#    df_mc = (pd.read_hdf('weighted_dfs/df_mc_{}_Iso_{}.h5'.format(EBEE,data_type))).reset_index(drop=True)
    ...
    for i in range(nTrials):
        for var in variables: 
            df_mc[f'{var}_corr_{i}'] = df_mc[var] + dfs_diff[i][f'{var}_corr_diff']
    print(df_mc.keys())
    print(df_mc)

    df_mc.to_hdf('{}/df_mc_{}_{}_corr_final_uncer.h5'.format(outdir,EBEE,data_type),'df',mode='w',format='t')
#    df_mc.to_hdf('{}/df_mc_{}_Iso_{}_corr_final_uncer.h5'.format(outdir,EBEE,data_type),'df',mode='w',format='t')
