...
def draw_plot(df_data, df_mc, x_vars, x_title, x_var_name, target, fig_name):
    ...
    for i in range(len(x_vars)-1):
        ...
        peak_true_mc[i] = len(df_mc_cut.query(f'p0t_{target} == 0'))/nTot_mc 
        tail_true_mc[i] = len(df_mc_cut.query(f'p0t_{target} != 0'))/nTot_mc 
...
def main(): 
    ...
    df_mc = (pd.read_hdf(f'dataframes/df_mc_{EBEE}_Iso_test.h5')).sample(nEvt, random_state=100).reset_index(drop=True)
    ...
    df_mc[f'p0t_{target}'] = np.apply_along_axis(lambda x: 0 if x==0 else 1, 0, df_mc[target].values.reshape(1,-1))
    ...
    draw_plot(df_data, df_mc, pTs,  '$p_T$',   'probePt',    target, f'{plotsdir}/check_clf_{EBEE}_{target}')
    draw_plot(df_data, df_mc, etas, '$\eta$',  'probeScEta', target, f'{plotsdir}/check_clf_{EBEE}_{target}')
    draw_plot(df_data, df_mc, rhos, '$\\rho$', 'rho',        target, f'{plotsdir}/check_clf_{EBEE}_{target}')
    draw_plot(df_data, df_mc, phis, '$\phi$',  'probePhi',   target, f'{plotsdir}/check_clf_{EBEE}_{target}')
