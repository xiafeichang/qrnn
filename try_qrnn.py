...
#    features = kinrho + variables[:variables.index(target)] 
    ...
    num_units_from = 1000
    shrink_rate = 0.8
    num_units = [num_units_from] + [int((num_units_from*shrink_rate**i)/len(qs)) for i in range(num_hidden_layers-1)]
    act = ['tanh'] + ['tanh' for _ in range(num_hidden_layers-1)]
#    dropout = [0.1 for _ in range(num_hidden_layers-1)]
#    gauss_std = None 

    history, eval_results = trainQuantile(X, Y, qs, qweights, num_hidden_layers, num_units, act, 
                                          opt = 'RMSprop',
                                          batch_size = batch_size, 
                                          epochs = 5000, 
                                          checkpoint_dir = 'ckpt/'+target, 
                                          save_file = 'models/{}_{}_{}'.format(data_key, EBEE, target), 
                                          evaluate_data = (X_test_raw, Y_test_raw))
