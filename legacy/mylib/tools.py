...
def parallelize2(func, X, Y, X_tail, *args, n_jobs=10, **kwargs): 
    if len(Y.shape) == 1: 
        Y = np.array(Y).reshape(-1,1)
    nY = Y.shape[-1]
    nX = X.shape[-1]
    Z = np.hstack([X,Y,X_tail])
    if nY > 1: 
        return np.concatenate(Parallel(n_jobs=n_jobs, verbose=20)(delayed(func)(sli[:,:nX], sli[:,nX:-nX], sli[:,-nX:], *args, **kwargs) for sli in np.array_split(Z, n_jobs)))
    else: 
        return np.concatenate(Parallel(n_jobs=n_jobs, verbose=20)(delayed(func)(sli[:,:nX], sli[:,nX], sli[:,-nX:], *args, **kwargs) for sli in np.array_split(Z, n_jobs)))
