...
class Shifter:
    ...
    def __init__(self,mcp0tclf,datap0tclf,mcq_model,X,Y,X_tail,qs,qweights,final_reg=False):
        self.qs = qs
        ...
        if final_reg:
            ...
            self.mcqtls = None
            self.X = np.array(X_tail)
        else:
            self.mcqtls   = np.array(predict(X_tail,qs,qweights,mcq_model)).T
            self.tailReg = None
...
def applyShift(X,Y,X_tail,mcp0tclf,datap0tclf,mcq_model,qs=None,qweights=None,final_reg=False):
    ...
    shifter = Shifter(mcp0tclf,datap0tclf,mcq_model,X,Y,X_tail,qs,qweights,final_reg)
    shifted_Y = shifter()
