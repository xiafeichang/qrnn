...
class Shifter2D:
    ...
    def __init__(self,mcp0tclf,datap0tclf,mcq_model0,mcq_model1,X,Y,X_tail,qs,qweights,final_reg=False):
        self.qs = qs
        ...
        if final_reg:
            ...
            self.mcqtls0 = np.array(predict(np.hstack((X_tail,Y[:,1].reshape(-1,1))),qs,qweights,mcq_model0)).T
            self.mcqtls1 = np.array(predict(np.hstack((X_tail,Y[:,0].reshape(-1,1))),qs,qweights,mcq_model1)).T
            self.tailReg0 = None
            self.tailReg1 = None
...
def apply2DShift(X,Y,X_tail,mcp0tclf,datap0tclf,mcq_model0,mcq_model1,qs=None,qweights=None,final_reg=False):
    ...
    return Shifter2D(mcp0tclf,datap0tclf,mcq_model0,mcq_model1,X,Y,X_tail,qs,qweights,final_reg)()
